...
- **Directory listing protocol:** keep `HandleReadDirectory()` on the proven legacy receive path: send the command response, reset the cart index, then emit the fixed 768-byte page while scanning for visible records. Do not reintroduce delayed-response/two-pass directory reads that hold sorted records in `sharedBuf` or perform extra SD opens during the C64 receive burst. The only listing cache allowed in `sharedBuf.ni` is the compact current-page `dirIdx` table used later by `INVOKE_WITH_INDEX`.
