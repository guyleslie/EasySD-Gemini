...
- **Directory navigation must use relative paths from root:** `sd.chdir()` then `sd.chdir("DIRNAME")` — absolute paths fail on nested paths. `GoBack()` derives the parent from `currentPath` and walks from root; do not rely on `sd.chdir("..")` because exFAT/newly-created directories may not provide a usable dot-dot entry.
...
- **SRAM overlay:** IO2 streaming, NI streaming, and command argument buffers share a single union (`sharedBuf` in CartApi.cpp). These are mutually exclusive at runtime, so `max(128, 400, 130) = 400 B` instead of `658 B`. `sharedBuf` is command-local scratch only: never store cross-command state in it. Persistent caches such as the directory page `dirIdx` cache must live in separate statics, otherwise the NI/sort/IO2 users can overwrite them. Any commit touching `sharedBuf`, `DirSortSlot`, or directory page caching must state which owner uses the bytes and why the lifetimes do not overlap.
