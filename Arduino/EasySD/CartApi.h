...
// TAP (standard only) conversion errors
// Note: error codes must stay in range $01-$7F (bit7 clear). Success is $80+.
#define TAP_UNSUPPORTED 0x12
#define TAP_BAD_TAP 0x13
#define TAP_WRITE_FAILED 0x14
...
// SPRINT 1: Reduced for ATmega328P memory constraints (orig: 128)
#define STREAMING_BUFFER_SIZE 64

// Size of each buffer used in double buffering (reduced from 400 to fit DEBUG mode)
// Sprint 1: MINIMAL for directory nav testing only (will restore for Sprint 2)
// NOTE: Streaming NOT used in Sprint 1 tests, so we minimize to save RAM
#define DOUBLE_BUFFER_SIZE 64

// Size of the non interrupted buffer, should be multiple of 8 (reduced from 400)
// Sprint 1: MINIMAL for directory nav testing (will restore for Sprint 2)
