...
**Output:**
- `build/easysd.prg` - Main C64 cartridge binary
...
**Output:**
- `build/easysd-debug.prg` - C64 debug binary with symbols
- `build/plugins/*.prg` - Plugin binaries
- `build/symbol/EasySDMenu.vs` - VICE-format labels (for binary monitor / `test_vice_menu.py`)
...
**Output:**
- `build/easysd-debug.prg` - C64 debug binary
- `build/plugins/*.prg` - Plugin binaries
...
**Output:**
- `build/easysd.prg` or `build/easysd-debug.prg`
...
**Parameters:**
- `target_name` - Build target identifier (e.g., "easysd", "arduino-compile")
...
**Description:** Copy artifacts from `build/artifacts/` to `Arduino/EasySD/`
**Copies:**
- FlashLib.h
- BuildConfig.h
